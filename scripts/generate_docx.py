...
"""Generate DOCX files from all Markdown sources in the Cardiology
...
HEADER_TEXT = "HCLS AI Factory \u2014 Cardiology Intelligence Agent"
...
EXPECTED_FILES = [
    ...
    "CARDIOLOGY_INTELLIGENCE_AGENT_RESEARCH_PAPER.md",
    "DEMO_GUIDE.md",
    "DEPLOYMENT_GUIDE.md",
...
def main():
    ...
    print(f"Processing {len(EXPECTED_FILES)} markdown files...\n")

    success = 0
    errors = []

    for md_name in EXPECTED_FILES:
        md_path = DOCS_DIR / md_name
        docx_name = md_name.replace(".md", ".docx")
        docx_path = DOCS_DIR / docx_name

        if not md_path.exists():
            msg = f"  SKIP  {md_name} (file not found)"
            print(msg)
            errors.append(msg)
            continue
    ...
    print(f"\nDone: {success}/{len(EXPECTED_FILES)} files converted.")
    if errors:
        print("Errors:")
        for e in errors:
            print(f"  {e}")
