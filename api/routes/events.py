"""Cardiology SSE (Server-Sent Events) streaming routes.

Provides real-time event streaming for long-running cardiology workflows,
risk calculations, and RAG query progress. Uses FastAPI's StreamingResponse
with text/event-stream content type for SSE protocol compliance.
...
def publish_event(event_type: str, data: dict) -> dict:
    ...
    This function is importable by other modules:
        ...
        publish_event("risk_calculation", {"score": 15.2, "category": "high"})

    Args:
        event_type: One of risk_calculation, workflow_complete,
                    critical_alert, gdmt_optimization, cross_modal_trigger.
    ...
    try:
        ...
        while (time.monotonic() - start) < max_duration:
            ...
            try:
                ...
                hb_seq += 1
                yield _sse_message("heartbeat", {"seq": hb_seq, "agent": "cardiology"}, event_id=f"hb-{hb_seq}")
...
async def _heartbeat_generator(
    ...
    while (time.monotonic() - start) < max_duration:
        seq += 1
        yield _sse_message("heartbeat", {"seq": seq, "agent": "cardiology"}, event_id=f"hb-{seq}")
...
async def _workflow_progress_generator(
    ...
    steps = {
        "cad": ["Validating inputs", "Querying guidelines", "Calculating ASCVD risk", "Analyzing imaging", "Generating recommendations"],
        "heart_failure": ["Classifying HF phenotype", "Evaluating GDMT status", "Checking device criteria", "Querying evidence", "Optimizing therapy"],
        "arrhythmia": ["Analyzing rhythm data", "Calculating CHA2DS2-VASc", "Assessing bleeding risk", "Querying EP evidence", "Generating plan"],
        "valvular": ["Grading severity", "Assessing intervention criteria", "Querying surgical risk", "Evaluating TAVR candidacy", "Summarizing"],
        "cardio_oncology": ["Reviewing chemotherapy agents", "Assessing baseline cardiac function", "Tracking GLS delta", "Evaluating biomarkers", "Risk stratifying"],
    }
