"""Prometheus metrics for the Cardiology Intelligence Agent.

Exposes counters, histograms, gauges, and info metrics for query latency,
collection hits, LLM token usage, risk score calculations, clinical workflow
executions, cross-modal triggers, GDMT optimisations, and ingest operations.

Scraped by the Grafana + Prometheus stack alongside existing HCLS AI Factory
exporters (node_exporter:9100, DCGM:9400).

All metrics use the ``cardio_`` prefix so they are easily filterable in
Grafana dashboards.
...
try:
    ...
    SEARCH_LATENCY = Histogram(
        "cardio_search_duration_seconds",
    ...
    EMBEDDING_LATENCY = Histogram(
        "cardio_embedding_duration_seconds",
    ...
    # ── LLM Metrics ────────────────────────────────────────────────────
    LLM_CALLS = Counter(
        "cardio_llm_calls_total",
        "Total LLM calls",
        ["model"],
    )

    LLM_LATENCY = Histogram(
        "cardio_llm_duration_seconds",
        "LLM call latency",
        ["model"],
    ...
    LLM_TOKENS = Counter(
        "cardio_llm_tokens_total",
        "Total LLM tokens",
        ["direction"],  # input / output
    )

    # ── Clinical Metrics ───────────────────────────────────────────────
    RISK_CALC_TOTAL = Counter(
    ...
    WORKFLOW_LATENCY = Histogram(
        "cardio_workflow_duration_seconds",
        "Clinical workflow execution time",
        ["workflow_type"],
        buckets=[0.5, 1.0, 2.5, 5.0, 10.0, 30.0, 60.0],
    )

    CROSS_MODAL_TRIGGERS = Counter(
        "cardio_cross_modal_triggers_total",
        "Cross-modal genomic triggers fired",
        ["trigger_type"],
    )

    GDMT_OPTIMIZATIONS = Counter(
        "cardio_gdmt_optimizations_total",
        "GDMT optimization runs",
    )

    CRITICAL_ALERTS = Counter(
        "cardio_critical_alerts_total",
        "Critical clinical alerts generated",
        ["alert_type"],
    )

    # ── Export Metrics ─────────────────────────────────────────────────
    EXPORT_TOTAL = Counter(
        "cardio_exports_total",
        "Report exports",
        ["format"],
    )
    ...
    INGEST_RECORDS = Counter(
        "cardio_ingest_records_total",
        "Total records ingested",
        ["collection"],
    )

    INGEST_ERRORS = Counter(
        "cardio_ingest_errors_total",
        "Total ingest errors",
        ["source"],
    )

    INGEST_LATENCY = Histogram(
        "cardio_ingest_duration_seconds",
    ...
    PIPELINE_STAGE_DURATION = Histogram(
        "cardio_pipeline_stage_duration_seconds",
    ...
    MILVUS_SEARCH_LATENCY = Histogram(
        "cardio_milvus_search_latency_seconds",
        "Milvus vector search latency",
        buckets=[0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.0],
    )

    MILVUS_UPSERT_LATENCY = Histogram(
        "cardio_milvus_upsert_latency_seconds",
    ...
    RISK_CALC_TOTAL = _NoOpLabeled()         # type: ignore[assignment]
    WORKFLOW_TOTAL = _NoOpLabeled()          # type: ignore[assignment]
    WORKFLOW_LATENCY = _NoOpLabeled()        # type: ignore[assignment]
    CROSS_MODAL_TRIGGERS = _NoOpLabeled()    # type: ignore[assignment]
    GDMT_OPTIMIZATIONS = _NoOpGauge()        # type: ignore[assignment]
    CRITICAL_ALERTS = _NoOpLabeled()         # type: ignore[assignment]
...
class MetricsCollector:
    """Convenience wrapper for recording Cardiology Intelligence Agent metrics.
    ...
    Usage::
        ...
        MetricsCollector.record_query("heart_failure", duration=1.23, success=True)
        MetricsCollector.record_search("cardio_literature", duration=0.15, num_results=12)
    ...
    def record_query(workflow_type: str, duration: float, success: bool) -> None:
        ...
        Args:
            workflow_type: Workflow category (e.g. ``"heart_failure"``).
    ...
    def record_embedding(duration: float) -> None:
        ...
        Args:
            duration: Embedding generation time in seconds.
        """
    ...
    @staticmethod
    def record_risk_calculation(score_type: str) -> None:
        """Record a risk score calculation.

        Args:
            score_type: Score type (e.g. ``"ascvd"``, ``"cha2ds2_vasc"``).
        """
        RISK_CALC_TOTAL.labels(score_type=score_type).inc()
    ...
    def record_workflow(workflow_type: str, duration: float) -> None:
        ...
        Args:
            workflow_type: Workflow type (e.g. ``"heart_failure"``).
    ...
    @staticmethod
    def record_cross_modal_trigger(trigger_type: str) -> None:
        """Record a cross-modal genomic trigger.

        Args:
            trigger_type: Trigger category (e.g. ``"imaging"``, ``"ecg"``).
        """
        CROSS_MODAL_TRIGGERS.labels(trigger_type=trigger_type).inc()

    @staticmethod
    def record_gdmt_optimization() -> None:
        """Record a GDMT optimisation run."""
        GDMT_OPTIMIZATIONS.inc()

    @staticmethod
    def record_critical_alert(alert_type: str) -> None:
        """Record a critical clinical alert.

        Args:
            alert_type: Alert category (e.g. ``"stemi"``, ``"cardiac_arrest"``).
        """
        CRITICAL_ALERTS.labels(alert_type=alert_type).inc()
    ...
    def record_export(format_type: str) -> None:
        ...
        Args:
            format_type: Export format (e.g. ``"markdown"``, ``"pdf"``,
                ``"json"``, ``"fhir_r4"``).
        """
    ...
    def record_ingest(
        ...
        Args:
            source: Data source (e.g. ``"pubmed"``, ``"clinical_trials"``).
            duration: Ingest time in seconds.
            record_count: Number of records ingested.
            collection: Target Milvus collection name.
            success: Whether the ingest completed without error.
        """
    ...
    def set_agent_info(
        ...
        Args:
            ...
            collections: Number of active Milvus collections.
            workflows: Number of registered clinical workflows.
        """
        AGENT_INFO.info(
            {
                "version": version,
    ...
    def set_milvus_status(connected: bool) -> None:
        ...
        Args:
            connected: Whether Milvus is currently reachable.
        """
    ...
    def update_collection_sizes(stats: Dict[str, int]) -> None:
        ...
        Args:
            stats: Mapping of collection name to row count.
        """
    ...
    def record_milvus_upsert(duration: float) -> None:
        ...
        Args:
            duration: Upsert time in seconds.
        """
